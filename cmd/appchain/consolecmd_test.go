...
const (
	ipcAPIs  = "admin:1.0 debug:1.0 miner:1.0 net:1.0 personal:1.0 appchain:1.0 rpc:1.0 txgen:1.0 txpool:1.0 web3:1.0"
	httpAPIs = "net:1.0 appchain:1.0 rpc:1.0 web3:1.0"
)
...
func TestConsoleWelcome(t *testing.T) {
	...
	//appchain.SetTemplateFunc("niltime", func() string { return time.Unix(0, 0).Format(time.RFC1123) })
...
func TestIPCAttachWelcome(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		...
		ipc = filepath.Join(ws, "appchain.ipc")
	}
	platon := runPlatON(t,
...
func testAttachWelcome(t *testing.T, platon *testplaton, endpoint, apis string) {
	// Attach to a running appchain note and terminate immediately
